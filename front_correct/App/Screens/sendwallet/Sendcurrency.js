...
import { addressValidation, evmEstimateGasFee } from "../../Network_controllers/EVM/Evm_contracthook";
...
export default function Sendcurrency({ navigation, route }) {
    ...
    const addressValidate = async () => {
        try {
            ...
            if (amount > routesdata.balance || amount.length == 0) {
                setAmountErr(true);
            }
    ...
    const Maximumamount = async () => {
        try {
            ...
            var maximum_amount = routesdata?.balance - (currencydata.tokenType == "BTC" ? 0.00007 : Gasprice?.gasFee)
    ...
    return (
        <SafeAreaView style={style.container} >
            ...
            <View onStartShouldSetResponder={() => Keyboard.dismiss()} style={{ flex: 1, backgroundColor: theme.background }}  >
                <Header title={`${currentChainconfig[routesdata?.currencyName]?.name} ${routesdata?.type == "Token" ? "- " + routesdata?.currency : ""}`} />
