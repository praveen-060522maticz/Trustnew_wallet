...
export const fiatcurrencies = async (currenciesdata, Fiatcurrencie) => {
  try {
    console.log('currenciesdata, Fiatcurrencie---->',currenciesdata, Fiatcurrencie);
...
export function navigateToExistingScreen(navigation, screenName, params = {}) {
  const routes = navigation.getState().routes;
  const existingIndex = routes.findIndex(r => r.name === screenName);

  if (existingIndex !== -1) {
    const screensToPop = routes.length - 1 - existingIndex;
    if (screensToPop > 0) {
...
var incLength = 2
export const CustomBackFunc = (navigation) => {
  try {
      const routes = navigation.getState().routes;;
