...
import { btcBalance } from '../BTC/Bitcoin_controller';
...
export const cryptoBlanace = async (privateKey, tokenType, address) => {
  console.log('cryptoBlanace---->',privateKey, tokenType, address);
  try {
    ...
    if (tokenType == 'TRX') {
      ...
      const bal = await btcBalance(address);
      console.log('address---->',address,bal);
      return bal;
    }
    else {
...
export const addressValidation = async (
  ...
  try {
    if (currency == "TRX") {
      ...
      // 
      const isValid = bitcore.Address.isValid(toAddress, currentChainconfig.BTC.envString);
      console.log('isavaliva---->',isValid,toAddress);
      return isValid;
    }
    else {
...
export const SendEVM = async (type, contractAddress, privKey, toaddress, tokenType, amount, decimal, Gas) => {
  try {
    console.log('type, contractAddress, privKey, toaddress, tokenType, amount, decimal, Gas---->',type, contractAddress, privKey, toaddress, tokenType, amount, decimal, Gas);
    let cryptobalance = await cryptoBlanace(privKey, tokenType);
    let fees = await calculate_gas(tokenType)
    console.log('cryptobalance---->',cryptobalance,fees?.fee,Number(fees?.fee / 1e18));
...
export const evmEstimateGasFee = async (data) => {
  try {
    ...
    return {
      gasFee: gasFee ? ((parseFloat(gasPrice) * parseFloat(gasFee)) / 10 ** 18).toFixed(5) : 0,
    };
  } catch (e) {
...
export const getCurrecnyBalance =async (tokenType, address) => {
  try {
    console.log('tokenType, address---->',tokenType, address);
    const web3 = await currentChainconfig[tokenType]?.web3_instance()
    let bal = await web3.eth.getBalance(address);
    console.log('baallalalal---->',bal);
    return formatEther(bal);
  } catch (error) {
    console.log('uesfuse---->',error);
  }
}